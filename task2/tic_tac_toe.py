...
def play_AI():
    ...
    def cell_selection_attack(amount, start_weight, end_weight):
        global weight_charts
        global computer_play
        global field
        x = computer_play
    ...
    def cell_selection_protection(amount, price):
        global weight_charts
        global person_play
        global field
        x = person_play
        ...
        def check_three_horizontal(row, col, x):
            ...
            if kol == amount:
                for i in [-1, 1]:
                    try:
                        if field[row][col + i]['text'] == ' ' and weight_charts[row][col + i] < price and (
                                col + i >= 0):
        ...
        def check_three_vertical(row, col, x):
            ...
            if kol == amount:
                for i in [-1, 1]:
                    try:
                        if field[row + i][col]['text'] == ' ' and weight_charts[row + i][col] < price and (
                                row + i >= 0):
        ...
        def check_three_diagonal_right(row, col, x):
            ...
            if kol == amount:
                for i in [-1, 1]:
                    try:
                        if field[row + i][col + i]['text'] == ' ' and weight_charts[row + i][col + i] < price and (
                                row + i >= 0) and (col + i >= 0):
    ...
    if game_run and not end_game:
        occupied_cells()
        start_attack()
        cell_selection_attack(1, 1, 2)
        cell_selection_attack(2, 2, 3)
        cell_selection_attack(3, 3, 5)
        cell_selection_attack(4, 5, 7)
        cell_selection_protection(2, 4)
        cell_selection_protection(3, 6)
        heaviest_weight()
